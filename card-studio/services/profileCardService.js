...
class ProfileCardService {
  ...
  async generate(opts = {}) {
    ...
    // 사진 → base64 data URI (빈 사진이면 그라데이션 배경 사용)
    const isEmptyPhoto = photoBuffer.length < 200; // 1x1 pixel placeholder
    const photoBase64 = isEmptyPhoto ? '' : this._toBase64DataUri(photoBuffer, photoMimeType);
    const gradientFallback = theme === 'light'
    ...
    const photoUrl = photoBase64 || gradientFallback;
