...
// ============================================
// PaceRise (실업연맹) 데이터 동기화
// ============================================

router.post('/pacerise/sync', async (req, res) => {
  try {
    const paceriseImporter = require('../services/paceriseImporter');
    const { competitionId, force } = req.body || {};

    if (competitionId) {
      // 특정 대회만 동기화
      const result = await paceriseImporter.importCompetition(Number(competitionId));
      res.json({ success: true, data: result });
    } else {
      // 전체 동기화
      const results = await paceriseImporter.importAll({ force: !!force });
...
router.get('/pacerise/status', async (req, res) => {
  try {
    ...
    res.json({
      ...
      data: {
        connection: health,
        remoteCompetitions: competitions.map(c => ({
          id: c.id, name: c.name, status: c.status,
          imported: existing.some(e => e.paceriseId === c.id),
        })),
        importedFiles: existing,
      },
    });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
