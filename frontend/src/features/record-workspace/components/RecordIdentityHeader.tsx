...
export function RecordIdentityHeader({
  ...
  const summary = context === 'workspace'
    ? `선택한 기록 후보 ${subjectCount}개에서 확인된 ${recordCount}개`
    : context === 'comparison'
      ? `비교 기록 후보 ${subjectCount}개에서 확인된 ${recordCount}개`
  ...
  const affiliationSummary = affiliationCount === undefined
    ? null
    : context === 'workspace'
      ? `선택한 기록 후보의 소속 ${affiliationCount}곳`
      : context === 'comparison'
        ? `비교 기록 후보의 소속 ${affiliationCount}곳`
