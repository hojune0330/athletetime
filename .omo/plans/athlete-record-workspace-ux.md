...
- [x] 5. 종목·시즌 기록 묶음과 상세 시트
  ...
  **완료 조건**:
  - [x] 종목·시즌 순서가 결정적이다.
  - [x] 낮을수록/높을수록 기록이 섞여도 화면 정렬이 날짜 기준에서 흔들리지 않는다.
  - [x] 풍속·예선·결승·부문·출처는 상세에서 확인 가능하다.
  - [x] 100개 행에서도 첫 화면은 종목 색인이고, 종목 진입 후
    한 시즌·10개 행만 먼저 보여 긴 스크롤을 줄인다.
  - [x] 선택 모드가 아닐 때 행별 다중 버튼이 없다.
  - [x] 상세 시트는 외부 상태로 제어해 Task 7의 주소 연동에서
    Back이 페이지보다 시트를 먼저 닫도록 연결할 수 있다.
  ...
  **검증 기록 (2026-07-31)**:
  - 4종목·8시즌·125개 픽스처로 결정적 묶음, 10→20개 점진 공개,
    상태 기록, 단일 행 동작, safe-area 하단 바를 검증했다.
  - 실제 공개 기록 41건으로 360x800·1280x900 브라우저 QA를 수행해
    종목 9개 색인, 첫 시즌 6행, 상세 시트, 선택 바, 수평 넘침 0,
    콘솔·페이지 오류 0을 확인했다.
  - `record=:publicRecordId`와 브라우저 Back 연동은 주소를 소유하는
    Task 7에서 검증하며, 이 Task에서는 제어형 시트 계약을 제공한다.
...
- [x] 6. 일반 검색 결과 V2
  ...
  **완료 조건**:
  - [x] 카드 한 번으로 상세 주소를 연다.
  - [x] 선택 모드는 명시적 진입 뒤에만 보인다.
  - [x] 최대 6개와 선택 해제가 즉시 반영된다.
  - [x] 일곱 번째 선택은 `한 번에 6개까지` 문구와 live region으로 안내된다.
  - [x] 하단 바는 모바일 탭바와 겹치지 않는 안전 영역을 가진다.
  ...
  **검증 기록 (2026-07-31)**:
  - 카드 정보는 이름·소속·시즌·기록 수로 제한하고, 평상 카드에는
    선수 상세 이동 하나만 남겼다.
  - 실제 공개 검색 후보 12명으로 360x800·1280x900 브라우저 QA를 수행해
    평상 카드의 상세 주소 이동, 명시적 묶기 진입, 선택 중 주소 불변,
    6명 유지, 일곱 번째 차단, 즉시 선택 해제, 수평 넘침 0,
    콘솔·페이지 오류 0을 확인했다.
  - 선택 변경은 `draftSubjectKeys` 계약으로만 내보내며 본인 기록·비교 상태와
    연결되는 표현이나 콜백은 새 컴포넌트에 두지 않았다.
...
- [x] 6A. 소속 통계 전용 탐색
  ...
  **결정**:
  - 소속 탐색은 개인 선수 담기·비교·본인 기록 흐름을 재사용하지 않는다.
  - 선수 검색의 기본 12명과 개인 기록 모음의 6명 한도를 소속 통계에 적용하지 않는다.
  - 원천 경기 결과의 소속 표기를 정규화한 뒤, 정확한 소속 단위로 전체 기록을 집계한다.
  - 지역명이 같은 다른 학교·팀은 합치지 않고 먼저 소속 후보로 나눈다.

  **표시**:
  - 개인 기록 행과 선수 상세 버튼 대신 선수 수, 기록 수, 대회 수, 종목 수를 보여준다.
  - 최근 8개 시즌의 기록·선수·대회·1~3위 표기 흐름과 상위 종목 구성을 보여준다.
  - `1~3위 표기`는 예선·결승 등을 포함한 원천 순위 필드 집계이며 공식 메달 집계로 부르지 않는다.
  - `모은 공개 기록 기준`, `공식 팀 명단·공식 입상 집계 아님`, `빠진 기록 가능`을 함께 표시한다.

  **검증 기록 (2026-07-31)**:
  - `진도` 검색에서 `진도군청`, `전남진도초등학교`, `강진도암중학교` 3개를 분리했다.
  - `진도군청`은 선수 검색 12명 제한을 넘는 19명과 138개 기록을 모두 집계했다.
  - 모바일 브라우저에서 후보 전환, 통계 요약, 시즌 막대, 수평 넘침 없음,
    개인용 기록 담기·비교 버튼 0개, 현재 콘솔 오류 0건을 확인했다.

  **Commit**: `feat(records): add affiliation statistics browsing`

- [ ] 6B. 팀 종류별 단일 팀 성과 대시보드

  **상세 계획**: `.omo/plans/team-performance-dashboard.md`

  **결정**:
  - `실업팀 · 대학팀 · 고등부 · 중등부 · 초등부`를 기록 부문 근거로 나눈다.
  - 한 팀을 누르면 `/records/teams/:teamKey` 독립 페이지로 이동한다.
  - 확인된 입상, 참가 대회, 선수 수, 모은 기록 기준 최고 갱신을 통계로 제공한다.
  - 예선·단계 미상·계주 중복을 분리하고 공식 메달 또는 개인 PB로 단정하지 않는다.
  - 팀 화면에는 개인 선수 이름과 개별 기록 목록을 넣지 않는다.

  **구현 순서**:
  1. 부문 판정기와 운영자 override
  2. 입상·참가·최고 갱신 계산기
  3. 팀 검색/상세 API
  4. 검색 category와 독립 팀 페이지
  5. 실데이터·모바일·신뢰 출시 게이트

  **완료 조건**:
  - [ ] 일반부만으로 실업팀을 강제 분류하지 않는다.
  - [ ] 예선 1위와 단계 미상 1~3위를 확인된 입상에서 제외한다.
  - [ ] 계주 한 팀을 선수 수만큼 중복 계산하지 않는다.
  - [ ] 첫 관찰 기록과 소속 변경 조각을 개인 최고 갱신으로 세지 않는다.
...
- [x] 7. 선수 전용 상세 페이지
  ...
  **완료 조건**:
  - [x] 다른 선수 상세 어디에도 `내 기록` 카드가 삽입되지 않는다.
  - [x] 80개 초과 시 절단 상태와 더 불러오기가 보인다.
  - [x] 네트워크 재시도 중 현재 경로와 선택이 유지된다.
  - [x] 찾을 수 없는 키는 검색·재시도 안내가 있는 복구 화면을 보인다.
  - [x] 첫 화면은 종목 색인이며 기록 전체를 한 번에 렌더하지 않는다.
  - [x] 직접 연 상세 주소의 뒤로 갈 기록이 없으면 `/records`로 복귀한다.
  - [x] 비교를 누르면 현재 선수 1명이 사전 선택되고 다른 선수 검색 입력에 초점이 간다.
  ...
  **검증 기록 (2026-07-31)**:
  - 125개 픽스처를 3페이지로 합쳐 중복 없이 125개, 마지막 페이지의 완전성 상태를 유지했다.
  - 남보하나 공개 기록으로 첫 화면 종목 색인, `3000m` 깊은 주소, 2025 시즌 7행,
    행 상세 주소, 정정 링크, Back 시 상세만 닫힘, 출처 탭 주소를 확인했다.
  - 비교 시작 뒤 `/records` 검색 입력이 실제 활성 요소가 되는 것을 확인했다.
  - 이용 불가 키에서 재시도와 기록 검색 복구 화면, 현재 콘솔 오류 0건을 확인했다.
...
- [x] 8. 검토 페이지와 기록 작업공간
  ...
  **완료 조건**:
  - [x] 같은 이름 키 2개는 검토 후 하나의 뷰에서 보인다.
  - [x] 다른 이름 키 2개는 확정 버튼이 비활성화되고 비교로 안내된다.
  - [x] 로컬 제외 후 새 API 호출에서도 해당 작업공간에서만 숨는다.
  - [x] 다른 탭·새 작업공간·원천 API에는 영향이 없다.
  - [x] 마지막 기록을 빼면 복구 가능한 빈 상태가 나타난다.
  - [x] 다중 키 화면은 현재·이전 소속을 단정하지 않고 상단 경고를 유지한다.
  - [x] 편집 바에는 비교가 없고 숨기기·취소만 있다.
  - [x] 20개 한도에서 자동 삭제 없이 저장한 모음 관리 화면으로 안내한다.
  - [x] undo는 토스트 시간이 지나도 다음 파괴적 동작 전까지 사용할 수 있다.
  - [x] 이용 불가 키는 과거 저장 이름·소속 없이 일반 문구로 표시된다.
  ...
  **검증 기록 (2026-07-31)**:
  - 실제 `남보하나` 후보 5명 중 3개 기록 묶음을 선택해 검토 페이지와 독립
    작업공간을 만들고, 41개 기록·6개 소속·8개 종목을 경계가 유지된 채 확인했다.
  - `3000m`에서 기록 2개를 로컬로 숨긴 뒤 소속 탭으로 이동해도 undo가 남았고,
    되돌리기·이름 변경·2단계 삭제·삭제 복구까지 동작했다.
  - `남보하나`와 `김국영`을 함께 선택하면 저장 동작 없이 비교 전환만 표시됐다.
  - `진도` 소속 탐색은 진도군청 19명·138건을 개인 행 없이 통계로만 표시했다.
  - 20개 한도, 전부 숨김, 이용 불가 키, 로컬 저장 차단은 자동 테스트로 검증했다.
  - 390x844 실제 브라우저에서 중첩 main을 제거하고 콘솔·페이지 오류 0건을 확인했다.
