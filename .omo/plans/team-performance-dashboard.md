...
- 팀 통계 필수 고지 4종, 금지 표현·개인 기능 8종, 접근성·카테고리·기간 선택
  계약을 자동 검사한다.
- 건국대학교(A)는 2009-2026 시즌 선택지를 제공한다. 최근 시즌 범위로 API를
  다시 요청해도 전체 확인 시즌 목록은 유지되므로 과거 시즌으로 이동할 수 있다.
- 실제 브라우저에서 건국대학교(A)의 2025 시즌을 선택한 뒤 새로고침해도 URL의
  `season=2025`, 2025 시즌 표기, 모은 기록 56건이 그대로 복원됐다.
- 360x800에서 본문 가로 넘침이 없고 주 본문 랜드마크는 하나다.
- 최종 새로고침 뒤 새 콘솔 오류와 경고는 각각 0건이다.
- 최종 전체 게이트는 사전 의미·카피 계약 22/22, 백엔드 322 통과·0 실패·
  원본 미보유 환경 5 skip, 실제 브라우저 흐름 1/1, 프론트 46/46, 타입 검사와
  프로덕션 빌드까지 통과했다.

다음 작업자는 PR의 검수와 출시 결정을 맡는다. 작업 1-4의 집계 의미를 변경할
때는 `team-category.test.js`와 `team-performance.test.js`를 먼저 갱신하고,
예선·단계 미상·계주 중복·첫 관찰 기록 방어 조건을 약화하지 않는다.
