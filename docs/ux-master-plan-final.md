...
| 항목 | 내용 | 상태 |
|---|---|---|
| 3A | 채팅「자유수다」오픈 — 단일방 활성화, 완전 익명(user_key_hash), 랜덤닉·리롤, DB 저장·30일 보존, 신고·블라인드·금칙어·규칙 동의·이용 제한 (설계: `docs/athletetime-chat-open-plan.md`) | ✅ 실사용 전환 — FeaturePreparingPage 제거, 웹소켓 클라+백엔드 레일 연결, 프로덕션 WS는 Render 직접(`wss://athletetime-backend.onrender.com/ws/chat`) |
