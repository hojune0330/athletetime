...
공개 검색과 정정 요청의 1차 요청 제한 키는 Express가 계산한 `req.ip`만 사용한다. 앱 코드가
`X-Forwarded-For` 헤더를 직접 신뢰하지 않는다. 현재 Render 운영 구조는 앱 앞에 신뢰하는 프록시가
한 단계 있다는 전제(`trust proxy = 1`)로 동작한다.

다음 중 하나가 바뀌면 기능 배포를 멈추고, 먼저 별도 검증 환경에서 프록시 체인을 확인한다.

1. CDN, WAF, 로드밸런서 또는 호스팅을 추가하거나 교체한다.
2. API 앞의 신뢰 프록시 단계 수가 달라진다.
3. API를 여러 인스턴스로 확장한다.

이유: 메모리 기반 제한은 한 서버 안에서만 계산된다. 여러 인스턴스에는 승인된 CDN/WAF 또는 공유
제한 저장소가 별도로 필요하다. 이 작업은 수집 범위나 이용자 공개 범위를 바꾸지 않지만, 운영 담당자가
공급자 설정과 제한 값을 기록한 뒤에만 진행한다. 브라우저나 클라이언트가 보낸 전달 헤더를 제한 우회
근거로 쓰지 않는다.
