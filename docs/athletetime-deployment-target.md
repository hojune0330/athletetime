...
- 데이터 정정·숨김 요청 접수
- 오픈 채팅「자유수다」(단일 상시방, DB 저장·30일 보존, 랜덤닉네임·신고·블라인드·금칙어·이용 제한 운영 안전장치 포함)
...
7. 직접 요청으로 커뮤니티·거래·업로드 쓰기가 거절되는지 확인하고, 채팅은 입장·발신·신고·블라인드 스모크가 동작하는지 확인한다. 채팅 상세 게이트는 `docs/athletetime-chat-open-plan.md` §5를 따른다.
